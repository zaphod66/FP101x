...
size :: Rose a -> Int
size (_ :> c) = 1 + sum [size s | s <- c]

leaves :: Rose a -> Int
leaves (_ :> c) = if ((length c) == 0) then 1
                  else sum [leaves s | s <- c]
...
instance Foldable Rose where
  fold (r :> c) = r `mappend` foldr (mappend) mempty [fold s | s <- c]
